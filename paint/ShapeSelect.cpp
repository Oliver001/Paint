...
///////////////
	...
	{
		pDC->StretchBlt(x1,y2,x2-x1,y1-y2,selectDC,selectRect.left,selectRect.top,selectRect.Width(),selectRect.Height(),SRCCOPY);
	}
	else if (mirror == 2)
	{
		pDC->StretchBlt(x2,y1,x1-x2,y2-y1,selectDC,selectRect.left,selectRect.top,selectRect.Width(),selectRect.Height(),SRCCOPY);
	}
	else //3
	{
		pDC->StretchBlt(x2,y2,x1-x2,y1-y2,selectDC,selectRect.left,selectRect.top,selectRect.Width(),selectRect.Height(),SRCCOPY);
	}
//	pDC->StretchBlt(x1,y1,x2-x1,y2-y1,selectDC,selectRect.left,selectRect.top,selectRect.Width(),selectRect.Height(),SRCCOPY);
...
void CShapeSelect::SetDC(CDC *pDC,int w,int h)
{
	...
	//你也可以用自己应该用的颜色  
	selectDC->FillSolidRect(0,0,1400,800,RGB(255,255,255));  
